...
void main()
{
	...
	//
  vec2 transformTex = vec2(TexCoord0.x + time * 0.005, TexCoord0.y - time * 0.007 );

	vec3 vMappedNormal = texture(uNormalMap, transformTex).rgb * 2.0 - 1.0;
	...
	vec3 vDiffuseColor = vec3( fDiffuse ) * texture( uDiffuseMap, transformTex ).rgb;
